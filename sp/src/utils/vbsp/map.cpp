...
//
// Purpose: Adds Parallax Corrected Cubemap code, with some changes from Mapbase.
//
...
// #include "matrixinvert.h" // Remove!
#include "mathlib/vmatrix.h" // Addition.
	...
	if (eResult == ChunkFile_Ok)
	{
		...
		if( !strcmp( "env_cubemap", pClassName ) )
		{
			if( ( g_nDXLevel == 0 ) || ( g_nDXLevel >= 70 ) )
			{
				...
				char* pParallaxObbStr = ValueForKey( mapent, "parallaxobb" ); // Addition.
				int size;
				size = IntForKey( mapent, "cubemapsize" );
				// Adds pParallaxObbStr to Cubemap_InsertSample.
		...
		if (!strcmp("parallax_obb", pClassName))
		{
			...
			*/
			...
			// Improve the matrix code by using built-in functions.
